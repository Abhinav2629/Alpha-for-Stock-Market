...
# --- SYSTEM ANCHOR (Python 3.13 Fix) ---
sys.modules['warnings'] = warnings 

# --- 1. SETTINGS & AGGRESSIVE MOBILE CSS (v31 Update) ---
...
st.markdown("""
    ...
    /* THE MOBILE SCROLL ENGINE (FROZEN & UPDATED) */
    @media (max-width: 768px) {
        ...
        div[data-testid="column"] {
            ...
            min-width: 90px !important; /* Adjusted for more columns */
            white-space: nowrap !important;
            margin-right: 15px !important;
        }
        div[data-testid="column"]:nth-child(1) { min-width: 130px !important; }
        /* POV Column remains wide */
        div[data-testid="column"]:nth-child(9) { min-width: 480px !important; white-space: normal !important; }
        .stSelectbox, .stButton, .stLinkButton { min-width: 120px !important; }
    }
    ...
    .val-label { font-size: 10px; color: #BDC3C7; }
    </style>
...
def analyze_v31(df, risk_val, alloc_val, mult, analysis_date):
    try:
        ...
        gc_note = "🌟 GC Active: Floor exists." if gc else "🌑 No GC: Resistance ahead."
        pov = f"**{state}** trigger (RSI: {rsi:.1f}). MTF Confluence: {mtf}%. {gc_note}"
...
for tab_idx, mode in enumerate(["Day Trading", "Swing Trading", "Positional", "Investors"]):
    with tabs[tab_idx]:
        bulk_data = yf.download(current_list, period="2y", interval="1d", group_by='ticker', auto_adjust=True, progress=False)
        
        # UPDATED HEADER WITH 52W COLUMNS
        h = st.columns([1.2, 0.6, 1, 1, 1, 1, 1, 1.8, 4.5, 1.5])
        h[0].write("**Stock**"); h[1].write("**Chart**"); h[2].write("**Price**")
        h[3].write("**52W High**"); h[4].write("**52W Low**") # NEW
        ...
        for i, symbol in enumerate(current_list):
            ...
            if data:
                ...
                # 52W Displays
                c[3].write(f"₹{data['hi52']:,.2f}")
                c[4].write(f"₹{data['lo52']:,.2f}")
                ...
                c[8].info(data['pov'])
                res = c[9].selectbox("Position", ["-", "Bought", "Sold"], key=f"sel31_{mode}_{i}")
